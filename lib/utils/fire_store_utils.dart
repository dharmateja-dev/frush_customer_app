...
class FireStoreUtils {
  ...
  static Future<UserModel?> getCurrentUser() async {
    // 1. First, try to get from the static variable (set by splash screen or successful login)
    if (currentUser != null) {
      return currentUser;
    }

    // 2. Second, try to get from SharedPreferencess (cached data)
    UserModel? userFromPrefs = Preferencess.getUserData();
    if (userFromPrefs != null &&
        userFromPrefs.id == FirebaseAuth.instance.currentUser?.uid) {
      currentUser = userFromPrefs; // Set static variable
      return currentUser;
    }

    // 3. Third, if not in prefs or static, check Firebase Auth and then fetch from Firestore
    ...
    try {
      ...
      if (snapshot.docs.isEmpty) {
        ...
        snapshot.docs.forEach((doc) {
          OnBoardingModel documentModel = OnBoardingModel.fromJson(doc.data());
    ...
    await for (final documentList in stream) {
      ...
      for (var document in documentList) {
        try {
          ...
          vendorList.add(VendorModel.fromJson(data));
        } catch (e) {
          debugPrint('getVendorsWithLocation parse error $e');
        }
      }
      break;
    }
  ...
  static Future getPaymentSettingsData() async {
    await fireStore
      ...
      if (value.exists) {
        ...
        await Preferencess.setString(
            Preferencess.payFastSettings, jsonEncode(payFastModel.toJson()));
      }
    });
    await fireStore
        .collection(CollectionName.settings)
      ...
      if (value.exists) {
        ...
        await Preferencess.setString(
            Preferencess.mercadoPago, jsonEncode(mercadoPagoModel.toJson()));
      }
    });
    await fireStore
        .collection(CollectionName.settings)
      ...
      if (value.exists) {
        ...
        await Preferencess.setString(
            Preferencess.paypalSettings, jsonEncode(payPalModel.toJson()));
      }
    });
    await fireStore
        .collection(CollectionName.settings)
      ...
      if (value.exists) {
        ...
        await Preferencess.setString(
            Preferencess.stripeSettings, jsonEncode(stripeModel.toJson()));
      }
    });
    await fireStore
        .collection(CollectionName.settings)
      ...
      if (value.exists) {
        ...
        await Preferencess.setString(
            Preferencess.flutterWave, jsonEncode(flutterWaveModel.toJson()));
      }
    });
    await fireStore
        .collection(CollectionName.settings)
      ...
      if (value.exists) {
        ...
        await Preferencess.setString(
            Preferencess.payStack, jsonEncode(payStackModel.toJson()));
      }
    });
    await fireStore
        .collection(CollectionName.settings)
      ...
      if (value.exists) {
        ...
        await Preferencess.setString(
            Preferencess.paytmSettings, jsonEncode(paytmModel.toJson()));
      }
    });
    await fireStore
        .collection(CollectionName.settings)
      ...
      if (value.exists) {
        ...
        await Preferencess.setString(Preferencess.walletSettings,
    ...
    await fireStore
      ...
      if (value.exists) {
        ...
        await Preferencess.setString(
            Preferencess.razorpaySettings, jsonEncode(razorPayModel.toJson()));
      }
    });
    await fireStore
        .collection(CollectionName.settings)
      ...
      if (value.exists) {
        ...
        await Preferencess.setString(
            Preferencess.codSettings, jsonEncode(codSettingModel.toJson()));
      }
    });
    ...
    await fireStore
      ...
      if (value.exists) {
        ...
        await Preferencess.setString(
            Preferencess.midTransSettings, jsonEncode(midTrans.toJson()));
      }
    });
    ...
    await fireStore
      ...
      if (value.exists) {
        ...
        await Preferencess.setString(
            Preferencess.orangeMoneySettings, jsonEncode(orangeMoney.toJson()));
      }
    });
    ...
    await fireStore
      ...
      if (value.exists) {
        ...
        await Preferencess.setString(
            Preferencess.xenditSettings, jsonEncode(xendit.toJson()));
      }
    });
  }
  ...
  static Future<List<ProductModel>> getProductByVendorId(
    ...
    String selectedFoodType = Preferencess.getString(
        Preferencess.foodDeliveryType,
  ...
  /// Check if a duplicate order exists for the same user, vendor, and scheduled time
  /// Returns: null if no duplicate, or the existing order ID if duplicate found
  static Future<String?> checkDuplicateOrder(
      {required String userId,
      required String vendorId,
      required Timestamp? scheduleTime,
      required bool isTakeAway}) async {
    try {
      Query query = fireStore
          .collection(CollectionName.restaurantOrders)
          .where('authorID', isEqualTo: userId)
          .where('vendorID', isEqualTo: vendorId)
          .where('takeAway', isEqualTo: isTakeAway);

      // For scheduled orders, check if exact time slot is booked
      if (scheduleTime != null) {
        query = query.where('scheduleTime', isEqualTo: scheduleTime);
      } else {
        // For immediate orders, check if any immediate order exists (no scheduleTime)
        query = query.where('scheduleTime', isNull: true);
      }

      // Only check non-cancelled/non-rejected orders
      ...
      log("Error checking duplicate order: $error");
      return null;
    }
  }
  ...
  static Future<String?> checkDuplicateDineInBooking(
      {required String userId,
      required String vendorId,
      required Timestamp bookingDate}) async {
    try {
      // Create time window: ±30 minutes from requested booking time
      final requestedDateTime = bookingDate.toDate();
      final timeWindowStart = Timestamp.fromDate(
          requestedDateTime.subtract(const Duration(minutes: 30)));
      final timeWindowEnd = Timestamp.fromDate(
      ...
      final snapshot = await fireStore
          ...
          .where('authorID', isEqualTo: userId)
          .where('vendorID', isEqualTo: vendorId)
          .where('date', isGreaterThanOrEqualTo: timeWindowStart)
          .where('date', isLessThanOrEqualTo: timeWindowEnd)
          .where('status', whereIn: [
            'pending',
            'confirmed',
            'approved',
            'Order Placed' // Match your status constant
          ])
          .limit(1)
          .get();
      ...
      log("Error checking duplicate dine-in booking: $error");
      return null;
    }
  }
  ...
  static Future<ReferralModel?> getReferralUserBy() async {
    ...
    try {
      await fireStore
          ...
          .get()
          .then((value) {
        // Check if document exists and has data before parsing
        if (value.exists && value.data() != null) {
          referralModel = ReferralModel.fromJson(value.data()!);
        }
      });
    } catch (e, s) {
      log('FireStoreUtils.getReferralUserBy $e $s');
      return null;
    }
    return referralModel;
  }
