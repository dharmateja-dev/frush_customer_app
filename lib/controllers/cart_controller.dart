...
class CartController extends GetxController {
  ...
  getCartData() async {
    ...
    selectedFoodType.value = Preferencess.getString(
  ...
  setOrder() async {
    ...
    final duplicateOrderId = await FireStoreUtils.checkDuplicateOrder(
        userId: FireStoreUtils.getCurrentUid(),
        vendorId: vendorModel.value.id!,
        scheduleTime: deliveryType.value == "schedule".tr
            ? Timestamp.fromDate(scheduleDateTime.value)
            : null,
        isTakeAway: selectedFoodType.value == "TakeAway");

    if (duplicateOrderId != null) {
      ShowToastDialog.closeLoader();
      ShowToastDialog.showToast(
          "You already have an active order for this restaurant at the same time. Please cancel your existing order before placing a new one."
              .tr);
      return;
    }
    ...
    await FireStoreUtils.getPaymentSettingsData().then(
      (value) {
        stripeModel.value = StripeModel.fromJson(
            jsonDecode(Preferencess.getString(Preferencess.stripeSettings)));
        payPalModel.value = PayPalModel.fromJson(
            jsonDecode(Preferencess.getString(Preferencess.paypalSettings)));
        payStackModel.value = PayStackModel.fromJson(
            jsonDecode(Preferencess.getString(Preferencess.payStack)));
        mercadoPagoModel.value = MercadoPagoModel.fromJson(
            jsonDecode(Preferencess.getString(Preferencess.mercadoPago)));
        flutterWaveModel.value = FlutterWaveModel.fromJson(
            jsonDecode(Preferencess.getString(Preferencess.flutterWave)));
        paytmModel.value = PaytmModel.fromJson(
            jsonDecode(Preferencess.getString(Preferencess.paytmSettings)));
        payFastModel.value = PayFastModel.fromJson(
            jsonDecode(Preferencess.getString(Preferencess.payFastSettings)));
        razorPayModel.value = RazorPayModel.fromJson(
            jsonDecode(Preferencess.getString(Preferencess.razorpaySettings)));
        midTransModel.value = MidTrans.fromJson(
            jsonDecode(Preferencess.getString(Preferencess.midTransSettings)));
        orangeMoneyModel.value = OrangeMoney.fromJson(jsonDecode(
            Preferencess.getString(Preferencess.orangeMoneySettings)));
        xenditModel.value = Xendit.fromJson(
            jsonDecode(Preferencess.getString(Preferencess.xenditSettings)));
        walletSettingModel.value = WalletSettingModel.fromJson(
            jsonDecode(Preferencess.getString(Preferencess.walletSettings)));
        cashOnDeliverySettingModel.value = CodSettingModel.fromJson(
            jsonDecode(Preferencess.getString(Preferencess.codSettings)));
        ...
        Stripe.merchantIdentifier = 'Sameats Customer';
